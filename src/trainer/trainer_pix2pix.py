...
from src.utilities.utility_pix2pix import logging_images, forward_test, forward_train



class Trainer_Pix2Pix():
    def __init__(self,strategy,vae,opt,produce_input_func):
        super(Trainer_Pix2Pix, self).__init__()
        ...
        self.produce_input_func = produce_input_func
        self.logging_func = logging_images

    def train_epoch(self, dataloader,vae=None, memory=None, use_memory=True):
        if vae==None:
            vae= self.vae
        if memory==None:
            memory  = self.strategy.memory
        ...
        model = vae.decoder.pix2pix_model
        ...
        for batch in pgbar:
            if use_memory:
                batch = memory.create_batch_data(batch, batch[0].shape[0])
            ...
            for index_sample_batch,index_dataset in enumerate(indices):#itero una per volta
                ...
                #Prepare real_A
                real_A, real_B = self.produce_input_func(self, self.strategy.complete_train_dataset, batch, index_sample_batch)

                fake_B = forward_train(model, real_A, real_B, model.device).cpu()

                # Log images
                self.logging_func(self,batch, index_sample_batch, real_A, real_B, fake_B, dataloader, self.strategy.complete_train_dataset, index_training, batch_index) 
        ...
        other_data_epoch = {  }         # x_hats,labels, zs, mus, logvars, losses "indices":lista_indices
        return metrics_epoch,other_data_epoch

    
    def test_epoch(self, dataloader,vae=None):
        if vae==None:
            vae = self.vae
        lista_zs,lista_indices,lista_x_hats,lista_mus,losses,lista_labels = [],[],[],[],[],[]
        ...
        for batch in pgbar:
            ...
            for index_sample_batch,index_dataset in enumerate(indices):#itero una per volta
                ...
                # Log images
                self.logging_func(self,batch, index_sample_batch, real_A, real_B, fake_B, dataloader,self.strategy.complete_test_dataset, index_training, batch_index) 
